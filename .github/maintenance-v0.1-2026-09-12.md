...
来源失败时保留上次成功统计并标记失败；来源身份、地址或格式变更后，不沿用旧来源的数据。至少一个来源本次成功才会生成新日期页面；全部失败时停止发布，不把旧统计包装成今日成功结果。程序、格式或数据异常导致步骤失败时，本次工作流停止自动提交，已有公开页面保持原样。

免费代理抓取、日期页面生成和原工具版本更新在同一次每日工作流中完成，再统一提交。手动运行以及相关程序/配置修改也可触发；不会每天重建新的 GitHub 仓库。
